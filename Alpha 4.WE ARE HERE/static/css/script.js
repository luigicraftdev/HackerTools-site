...
let currentES = null;
  ...
  async function identifyHash() {
    ...
    if (!data.identified) {
      res.innerHTML = '<div class="no-match">⚠ Formato hash non riconosciuto. Controlla l\'input.</div>';
      return;
    }

    let html = `<div class="hash-meta" style="margin-top:0.75rem;margin-bottom:0.75rem;">
      Lunghezza: <strong>${data.length}</strong> caratteri &nbsp;|&nbsp; ${data.matches.length} tipo/i rilevato/i
    </div>`;
    ...
    data.matches.forEach((m, i) => {
      ...
      <div class="hash-match" style="animation-delay:${i*0.08}s">
        ...
        <div style="display:flex;gap:0.5rem;align-items:center;flex-wrap:wrap;">
          ...
          ${m.crackable ? `<button class="crack-btn-inline" onclick="setupCrack('${m.name}')">[ CRACKA ]</button>` : ''}
        </div>
      </div>`;
    });
